...
namespace SDO.SnipsAI.Client.Test.Hermes.Nlu
{
    ...
    public class InstantTimeTests
    {
        ...
        {
            var instance = JsonConvert.DeserializeObject<ValueBase>("{ \"kind\": \"InstantTime\", \"value\": \"2017-06-13 07:00:00\", \"grain\": \"Hour\", \"precision\": \"Exact\" }");
            Assert.IsNotNull(instance);

            var parsedInstance = instance as InstantTime;
            Assert.IsNotNull(parsedInstance);

            Assert.AreEqual(new DateTime(2017, 06, 13, 07, 00, 00, DateTimeKind.Local), parsedInstance.Value);
